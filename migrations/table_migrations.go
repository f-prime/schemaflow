...
func (x *DiffableTable) GetCurrentNames() []string {
  return buildStringArray[PgTable](x.current_tables, func(t PgTable) string {
    return t.relnamespace + "." + t.relname
  })
}

func (x *DiffableTable) GetNewNames() []string {
  return buildStringArray[PgTable](x.new_tables, func(t PgTable) string {
    return t.relnamespace + "." + t.relname
  })
}

func (x *DiffableTable) GenerateAddStmts(ctx *core.Context) []string {
  return buildStringArray[string](computeAddedObjects(x), func (x string) string {
    return fmt.Sprintf("CREATE TABLE %s()", x)
  })
}

func (x *DiffableTable) GenerateDropStmts(ctx *core.Context) []string {
  return buildStringArray[string](computeRemovedObjects(x), func (x string) string {
    return fmt.Sprintf("DROP TABLE %s", x)
  })
}

func (x *DiffableTable) GenerateUpdateStmts(ctx *core.Context) []string {
  var stmts []string 
